...
// MediaSFU Webinar view component
...
const returnUI = true // Flip to false to wire up a bespoke webinar UI

const cardOverrides = createCardOverrides({
  accent: '#8b5cf6',
  glow: 'rgba(139, 92, 246, 0.35)',
  audioBackground: 'rgba(196, 181, 253, 0.25)',
  miniBackground: 'rgba(167, 139, 250, 0.22)',
  miniTextColor: '#2e1065',
})
...
if (useSeed) {
  ...
  const participants_ = generateRandomParticipants({
    ...
    coHost: '',
    host: hostName,
    forChatWebinar: eventType === 'webinar' || eventType === 'chat',
  });
...
// MediasfuWebinar, MediasfuBroadcast, MediasfuConference, MediasfuChat
//
// By default, we'll use MediasfuWebinar with custom settings.
...
<template>
  <!--
    ...
    The MediasfuWebinar component is used by default.
