...
## Build real-time communication into Vue

MediaSFU Vue combines a Vue 3 WebRTC client, Composition API-friendly components, prebuilt room experiences, and lower-level customization APIs for video meetings, voice and video calls, webinars, interactive livestreams, chat, recording, screen sharing, whiteboards, polls, live captions, translation, and AI-assisted communication.
...
| A fully branded Vue collaboration workspace | `ModernMediasfuGeneric` or headless mode | `uiOverrides`, custom cards, `sourceParameters` |
...
Use the exported composable when Vue should render the whole room while
MediaSFU continues to own signaling, transports, participant state, and media
lifecycle.
...
<template>
  ...
  <p>{{ room.ready.value ? 'Room ready' : 'Connecting…' }}</p>
  <p>{{ room.participants.value.length }} participants</p>
  <button type="button" @click="room.controls.toggleMic()">Microphone</button>
  <button type="button" @click="room.controls.toggleCamera()">Camera</button>
  <button type="button" @click="room.controls.leave()">Leave</button>

  <!-- Render every entry, even inside a visually hidden container. -->
  <component
    :is="audio"
    v-for="(audio, index) in room.audioComponents.value"
    :key="index"
  />
</template>
```

`sourceParameters` is a stable seed. `parameters` is a `shallowRef` containing
the newest published bag. The readiness, media, participant, permission, and
session fields are computed from that newest bag. Bind `mediaChanged` as shown
instead of polling.

Do not call `getUpdatedAllParams()` during render, a computed getter, a watcher,
or a timer; it publishes. Pure reads use `getCurrentParams()`. Replace the
example URL with your server, and keep reusable cloud credentials behind an
authenticated backend before release.
...
### Is MediaSFU Vue only a UI component package?

No. It includes prebuilt Vue room UI and the browser-side WebRTC/session runtime. A MediaSFU-compatible backend still provides signaling, room lifecycle, and SFU media routing.

### Can I keep my existing Vue design system?

Yes. Replace targeted surfaces with `uiOverrides`, custom cards, and custom components, or use `:returnUI="false"` to place the runtime behind a completely custom Vue interface.

### Can I self-host MediaSFU?

Yes. Pass `localLink` for a MediaSFU Open deployment, or use MediaSFU Cloud when you prefer managed room and media infrastructure.

### Which collaboration features are included?

The SDK supports meeting, webinar, broadcast, and chat experiences with screen sharing and annotation, recording, whiteboards, polls, breakout rooms, live captions, translation, and extensible AI-agent workflows.
...
## Related MediaSFU capabilities: AI phone agents, SIP, and PSTN

The wider MediaSFU platform also supports SIP/PSTN calling and AI phone-agent workflows, including provider integrations, human handoff, analytics, and transcription. These services are separate from the Vue browser SDK but can complement the same communication product. See the [MediaSFU telephony documentation](https://mediasfu.com/telephony).
...
3. If you wrap a modal or embedded sidebar body, resolve live params from `parameters.getCurrentParams?.()` when it exists instead of caching the first `parameters` snapshot. That keeps device lists, recording state, and other mutable room data current without publishing during render.
